...
<script>
import Vue from 'vue'
import SpoEntityItem from '~/components/molecules/SpoEntityItem'
Vue.use(require('vue-shortkey'))
export default {
  components: { SpoEntityItem },
  props: {
    text: {
      type: String,
      default: '',
  ...
  methods: {
    ...
    validateSpan() {
      ...
      for (const entity of this.entities) {
        if (entity.start_offset <= this.start && this.start < entity.end_offset) { return false }
        if (entity.start_offset < this.end && this.end <= entity.end_offset) {
          return false
        }
        if (this.start < entity.start_offset && entity.end_offset < this.end) { return false }
      }
      return true
    },
    open(e) {
      this.setSpanInfo()
      if (this.validateSpan()) this.show(e)
    },
    assignLabel(labelId) {
